...
module Exercises = struct
  ...
  let rec minimax (node : Game.t) depth maximizingPlayer me =
    let terminal_found =
      match evaluate node with
      | Game_continues -> false
      | _ -> true
    in
    match Int.equal depth 0 || terminal_found with
    | true -> score node me
    | false ->
      let possible_positions = available_moves node in
      if maximizingPlayer
      then (
        let value = Float.neg_infinity in
        let move_scores =
          List.map possible_positions ~f:(fun position ->
            (* print_endline ("Calling minimax with depth: " ^ Int.to_string
               depth ^ ", " ^ (Int.to_string ((Map.length
               child_game.board)))); *)
            let child_game = place_piece node ~piece:me ~position in
            minimax child_game (depth - 1) false (Game.Piece.flip me))
          |> List.sort ~compare:(fun a b -> -1 * Float.compare a b)
        in
        let value = Float.max (List.hd_exn move_scores) value in
        value)
      else (
        let value = Float.infinity in
        let _opponent = Game.Piece.flip me in
        let move_scores =
          List.map possible_positions ~f:(fun position ->
            let child_game = place_piece node ~piece:me ~position in
            minimax child_game (depth - 1) true (Game.Piece.flip me))
          |> List.sort ~compare:Float.compare
        in
        let value = Float.min (List.hd_exn move_scores) value in
        value)
  ;;
...
let handle_turn (_client : unit) (query : Rpcs.Take_turn.Query.t) =
  let result =
    Or_error.try_with ~backtrace:true (fun () ->
      ...
      let best_play =
        match Exercises.winning_moves ~me game with
        | [] ->
          let possible_positions =
            match
              Exercises.available_moves_that_do_not_immediately_lose me game
            with
          ...
          let play_scores =
            List.map possible_positions ~f:(fun position ->
              let possible_gamestate =
                Exercises.place_piece game ~piece:me ~position
              in
              let gamestate_score =
                Exercises.minimax possible_gamestate 9 true me
              in
              position, gamestate_score)
          in
          let sorted_games =
            List.sort play_scores ~compare:(fun (_, score_a) (_, score_b) ->
              Float.compare score_b score_a)
          in
          Tuple2.get1 (List.hd_exn sorted_games)
        | _ -> List.hd_exn (Exercises.winning_moves ~me game)
      in
      let response =
